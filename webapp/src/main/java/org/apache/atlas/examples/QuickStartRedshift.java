 ...
 */
public class QuickStartRedshift {
    ...
    static void runQuickstart(String[] args, String[] basicAuthUsernamePassword) throws Exception {
        ...
        QuickStartRedshift quickStartRedshift = null;
        try {
            if (!AuthenticationUtil.isKerberosAuthenticationEnabled()) {
                quickStartRedshift = new QuickStartRedshift(urls, basicAuthUsernamePassword);
            } else {
                quickStartRedshift = new QuickStartRedshift(urls);
            }
            ...
            //quickStartRedshift.search();

            // Shows some lineage information on entity
            //quickStartRedshift.lineage();
        } finally {
            if (quickStartRedshift!= null) {
                quickStartRedshift.closeConnection();
            }
        }
    ...
    static String[] getServerUrl(String[] args) throws AtlasException {
        ...
        if (ArrayUtils.isEmpty(urls)) {
            System.out.println("org.apache.atlas.examples.QuickStartRedshift <Atlas REST address <http/https>://<atlas-fqdn>:<atlas-port> like http://localhost:21000>");
            System.exit(-1);
        }
    ...
    QuickStartRedshift(String[] urls, String[] basicAuthUsernamePassword) {
        atlasClientV2 = new AtlasClientV2(urls,basicAuthUsernamePassword);
    }

    QuickStartRedshift(String[] urls) throws AtlasException {
